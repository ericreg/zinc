...
async fn concurrency_non_deterministic_01_spawn_print_race__greet_i64(x: i64) {
    println!("{}", x);
}
...
async fn main() {
    ...
    __zinc_spawn_handles.push(tokio::spawn(async move { concurrency_non_deterministic_01_spawn_print_race__greet_i64(42).await; }));
    println!("done");
