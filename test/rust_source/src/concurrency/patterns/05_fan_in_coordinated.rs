...
async fn main() {
    ...
    __zinc_spawn_handles.push(tokio::spawn({ let __zinc_spawn_arg_0 = left.clone(); async move { concurrency_patterns_05_fan_in_coordinated__send_left_Channel(__zinc_spawn_arg_0).await; } }));
    __zinc_spawn_handles.push(tokio::spawn({ let __zinc_spawn_arg_0 = right.clone(); async move { concurrency_patterns_05_fan_in_coordinated__send_right_Channel(__zinc_spawn_arg_0).await; } }));
