...
async fn concurrency_channels_02_spawn_parameter_round_trip__tx_i64_Channel(x: i64, send_x: __ZincChannel<i64>) {
    send_x.send(x).await;
}
...
async fn main() {
    ...
    let x_chan = __ZincChannel::<i64>::unbounded();
    __zinc_spawn_handles.push(tokio::spawn({ let __zinc_spawn_arg_1 = x_chan.clone(); async move { concurrency_channels_02_spawn_parameter_round_trip__tx_i64_Channel(42, __zinc_spawn_arg_1).await; } }));
